"""SCTools: Pegasus/Scanpy utilities and on-disk AnnData helpers.

This module started as a collection of local Pegasus extensions and has been
refined into a reusable toolkit. Lightweight H5AD utilities are importable
without Pegasus or Scanpy; analysis and plotting helpers load optional
...
def mark_MitoCarta(data, mitocarta_path=None, gene_column="Symbol"):
    ...
    ----------
    data
        Pegasus or AnnData-like object with `.var`.
    mitocarta_path
        Path to a MitoCarta CSV file. If omitted, the function first tries the
        original lab-specific path used by this script.
    gene_column
        Column in the CSV containing gene symbols.
    """
...
def read_everything_but_X(pth) -> ad.AnnData:
    ...
    This is useful for large on-disk manipulations where annotations are needed
    but the count matrix should remain untouched.
    """
...
def concat_on_disk(input_pths, output_pth, temp_pth='temp.h5ad'):
    ...
    with h5py.File(output_pth, 'a') as target:
        ...
        # initiate empty X
        dummy_X = _empty_csr(n_variables)
...
def write_h5ad_with_new_annotation(orig_h5ad, adata, new_h5ad, raw = True):
    ...
    with h5py.File(new_h5ad, "a") as target:
        with h5py.File(orig_h5ad, "r") as src:
            dummy_X = _empty_csr(adata.var.shape[0])
            write_elem(target, "X", dummy_X)
